...
export function Toolbar() {
  ...
  const handleExportPNG = useCallback(async () => {
    ...
    if (!el) return;
    try {
      const dataUrl = await toPng(el, { cacheBust: true, backgroundColor: theme === 'dark' ? '#18181b' : '#f8fafc' });
      const a = document.createElement('a');
      a.href = dataUrl;
  ...
  const handleExportSVG = useCallback(async () => {
    ...
    if (!el) return;
    try {
      const dataUrl = await toSvg(el, { cacheBust: true, backgroundColor: theme === 'dark' ? '#18181b' : '#f8fafc' });
      const a = document.createElement('a');
      a.href = dataUrl;
      a.download = 'knowledge-graph.svg';
      a.click();
    } catch (err) {
      setError(`SVG export failed: ${err instanceof Error ? err.message : String(err)}`);
    }
  }, [theme, setError]);
    ...
    <Panel position="top-left" className="!m-3">
      <div className="flex flex-col w-40 kg-surface border kg-border rounded-lg shadow-xl overflow-hidden">
        ...
        <div className="flex flex-col p-1.5">
          ...
          <ToolbarButton onClick={handleExportPNG} label="Export PNG">
            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          <ToolbarButton onClick={handleExportSVG} label="Export SVG">
            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <div className="px-1.5 pb-1.5">
          <Divider />
          <a
            href="https://github.com/RishabhSinha07/kg-explorer"
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2.5 w-full px-2.5 py-1.5 rounded-md kg-text-muted kg-hover hover:!text-[var(--kg-text)] transition-all duration-150 text-left"
          >
